...
void AES_invSubBytes( uint8_t s[4][Nb] )
{
    for( int r = 0; r < 4; r++) {
        for( int c = 0; c < Nb; c++) {
            s[r][c] = AES_inverseSubstitute( s[r][c] );      
        }
    }
    return;
}
...
{
    return INVERSE_SBOX[ (a >> 4) & 0x0F ][ a & 0x0F ];
}
...
void AES_invMixColumns( uint8_t s[4][Nb] )
{
    ...
    for( int c = 0; c < Nb; c++) {
        s[0][c] = AES_gf256Mul( t[0][c], 0x0e ) ^ AES_gf256Mul( t[1][c], 0x0b ) ^ AES_gf256Mul( t[2][c], 0x0d ) ^ AES_gf256Mul( t[3][c], 0x09 );
        s[1][c] = AES_gf256Mul( t[0][c], 0x09 ) ^ AES_gf256Mul( t[1][c], 0x0e ) ^ AES_gf256Mul( t[2][c], 0x0b ) ^ AES_gf256Mul( t[3][c], 0x0d );
        s[2][c] = AES_gf256Mul( t[0][c], 0x0d ) ^ AES_gf256Mul( t[1][c], 0x09 ) ^ AES_gf256Mul( t[2][c], 0x0e ) ^ AES_gf256Mul( t[3][c], 0x0b );
        s[3][c] = AES_gf256Mul( t[0][c], 0x0b ) ^ AES_gf256Mul( t[1][c], 0x0d ) ^ AES_gf256Mul( t[2][c], 0x09 ) ^ AES_gf256Mul( t[3][c], 0x0e );   
    }
    return;
}